...
static int GFX_get_pos(GFX* gfx, GFX_CURSOR* cursor)
{
...
static int GFX_set_pos(GFX* gfx, GFX_CURSOR* cursor)
{
	int l_ret = GFX_FAILED;
...
static int GFX_check_boundaries(const GFX* gfx, const GFX_CURSOR* cursor)
{
...
static int GFX_update_buff(GFX* gfx, char* buf, GFX_CURSOR* cursor, uint16_t len)
{
	uint16_t start_pos;
	uint16_t end_pos;
